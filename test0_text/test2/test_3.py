...
def login(usname,pwd):  #定义一个登录函数，两个形参
    response={'message':None,'code':None,'Token':None}   #先定义空的，然后符合对应条件返回内容

    if usname=='lcy123'  and  pwd==123456:   #if 判断  符合条件，返回对应内容
        response['message']='success'  #通过key=定义
...
class Testlogin(unittest.TestCase):   #定义TestLoin类，继承unittest.TestCase

    def test_login1(self):   #定义一个实例方法，和上面不一样，上面是类
        actual=login(case1['usname'],case1['pwd'])   #方法里面调用login函数  输入用例名，获取对应用例的实际结果
